...
- `/pr-draft` → abre PR inicial em draft a partir do plano (branch + empty commit + title/body do SPEC), devolve o root pra branch default e cria worktree via `/git-worktree`. Bloqueado pra criar PR → instrui o usuário comando-por-comando. `/pr-draft sync` reescreve o body pós-implementação como prévia pro reviewer (O quê / Por quê / Como / Test plan, rastreável a SPEC/IMP/diff). **Nunca commita código nem sai de draft sozinho**
