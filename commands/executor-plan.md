...
- **Hoist a promise**: dispare a chamada assim que os inputs existem e faca `await` so no ponto onde o resultado e usado. Ex.: `const userP = fetchUser(id)` no topo, `const user = await userP` mais abaixo — o request roda em paralelo com o codigo entre os dois.
  - **CUIDADO — so hoist se a operacao for independente do que roda no meio.** O codigo entre o disparo e o `await` NAO pode afetar o resultado da promise em voo. Sinais de que NAO da pra hoist: (a) o meio faz um **write** que a leitura hoisted deveria enxergar (read-your-writes — voce le stale); (b) ordem/causalidade importa (a promise depende de algo que so fica pronto no meio); (c) o meio muta input/estado compartilhado que a promise le; (d) **transaction boundary** — a chamada precisa rodar dentro/depois de uma transacao aberta no meio; (e) **erro** — promise que rejeita e so e aguardada la embaixo estoura num ponto inesperado, e se algum caminho pular o `await`, vira unhandled rejection. Na duvida, deixe sequencial — correcao > concorrencia.
- **`Promise.all`** quando voce precisa de varios resultados antes de seguir: `const [a, b] = await Promise.all([fetchA(), fetchB()])` em vez de `const a = await fetchA(); const b = await fetchB()`. So quando as chamadas sao mutuamente independentes (uma nao consome o resultado da outra).
...
- **Concorrencia de I/O**: paralelize chamadas assincronas independentes (hoist da promise ou `Promise.all`) em vez de `await` sequencial — mas **so hoist quando o codigo do meio nao afeta o resultado** (read-your-writes, ordem, transacao, rejeicao tardia = deixe sequencial). MAS nunca fan-out de query por item (round-robin/N+1) — batch com `WHERE IN`/join; limite concorrencia quando escala com os dados. Vale tambem pros subagentes `[P]` (inclua a diretriz no prompt deles)
